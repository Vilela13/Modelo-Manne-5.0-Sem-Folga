/*
 * VariaveisFixas.hpp
 *
 *  Created on: 16/04/2015
 *      Author: mateus
 */

#ifndef VARIAVEISFIXAS_HPP_
#define VARIAVEISFIXAS_HPP_


// As unidades do problema são km e horas.

#define NumeroDePlantasVariaveisFixas 2
#define NumeroDeEntregasVariaveisFixas 4
#define NumeroDeVeiculosVariaveisFixas 5
#define VelocidadeVariaveisFixas 20
#define CapacidadeCaminhao 10
#define TempoDeVidaConcretoVariaveisFixas 1.5


#endif /* VARIAVEISFIXAS_HPP_ */
